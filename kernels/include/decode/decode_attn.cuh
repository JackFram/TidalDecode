...
#pragma unroll
		...
		if constexpr(output_qk_product) {
 ...
 * \param qk_product The product of q and k, [num_qo_heads, kv_len]
 * \param sm_scale A float indicates the scale applied to pre-softmax logits
...
BatchDecodeWithPagedKVCacheKernel(DTypeIn* __restrict__ q,
								  ...
								  DTypeOut* __restrict__ qk_product,
								  float sm_scale,
	...
	const uint32_t max_seq_len = paged_kv.max_seq_len;
	...
	if constexpr(output_qk_product) {
		if constexpr(partition_kv){
			qk_product_ptr = qk_product + (kv_partition_info.batch_idx_map[batch_idx] * num_qo_heads + qo_head_idx) * max_seq_len;
		} else {
			qk_product_ptr = qk_product + (batch_idx * num_qo_heads + qo_head_idx) * max_seq_len;
		}
	}
 ...
 * \param lse The logsumexp values.
 * \param num_qo_heads A integer indicates the number of heads of query and output
 * \param qk_product The product of query and key matrix
 * \param rotary_mode The rotary mode
		...
		{SWITCH_HEAD_DIM(
			head_dim, HEAD_DIM, {SWITCH_ROTARY_MODE(rotary_mode, ROTARY_MODE, {
				return BatchDecodeWithPagedKVCacheDispatched<GROUP_SIZE,
															 ...
															 DTypeOut,
															 IdType>(
					q, paged_kv, kv_partition_info, o, tmp, lse, qk_product, rope_scale, rope_theta, stream);
			})})});
